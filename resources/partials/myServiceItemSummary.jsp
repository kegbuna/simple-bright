  ...
  <% com.kd.arsHelpers.SimpleEntry entry=(com.kd.arsHelpers.SimpleEntry)resultsList.get(0); %>
    <tr class="serviceItemStatistic">
      ...
      <%String openCount = (String)entry.getEntryFieldValue("700001001");
	      ...
	      <td width="19%" class="statItem"> <span style="display:block;" class='summaryItem' onclick='catalogUtils.showSelectedRequests(true, false, this)'>In Progress(<%= entry.getEntryFieldValue("700001001")%>)</span></td>
      <%}%>
      <%String closedCount = (String)entry.getEntryFieldValue("700001002");
	      ...
	      <td width="19%" class="statItem"> <span style="display:block;" class='summaryItem' onclick='catalogUtils.showSelectedRequests(false, false, this)'>Closed(<%= entry.getEntryFieldValue("700001002")%>)</span></td>
      <%}%>
      <%String forAppCount = (String)entry.getEntryFieldValue("700001003");
      	if (forAppCount != null && forAppCount.trim().length() >0 && !forAppCount.trim().equals("0")) { %>
      		<td width="19%" class="statItem"> <span style="display:block;" class='summaryItem' onclick='catalogUtils.showSelectedRequests(true, true, this)'>Needing approval(<%= entry.getEntryFieldValue("700001003")%>)</span></td>
      <%}%>
      <%String pendCount = (String)entry.getEntryFieldValue("700001006");
	      ...
	      <td width="16%" class="statItem"> <span style="display:block;" class='summaryItem' onclick='var options = {isOpen: true, isApproval: true, isActionPending: true, clickedEl: this}; catalogUtils._showSelectedRequests(options)'>Pending Actions(<%= entry.getEntryFieldValue("700001006")%>)</span></td>
      <%}%>
      <%String appCount = (String)entry.getEntryFieldValue("700001005");
      	if (appCount != null && appCount.trim().length() >0 && !appCount.trim().equals("0")) { %>
	      <td width="19%" class="statItem"> <span style="display:block;" class='summaryItem' onclick='catalogUtils.showSelectedRequests(false, true, this)'>Completed Approval(<%= entry.getEntryFieldValue("700001005")%>)</span></td>
      <%}%>
    </tr>
    </table>
<%} %>
