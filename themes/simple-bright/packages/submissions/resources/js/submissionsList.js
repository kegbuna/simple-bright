...
var displayFields = {
    ...
    'Submitted': '<i class="fa fa-clock-o fa-2x"></i>',
...
function defaultRowCallback(li, record, index, displayFields) {
    ...
    //Setting some size classes for the label and value
    $('.submitted-label').addClass('col-sm-1');
    $('.submitted-value').addClass('col-sm-4');
    ...
    if(record['Customer Survey Status'] !== 'In Progress' && record['Submit Type'] !== 'Approval') {
        innerRightColumn.prepend('<br />')
            .prepend(
                $('<a>').addClass('view-activity btn btn-default ')
