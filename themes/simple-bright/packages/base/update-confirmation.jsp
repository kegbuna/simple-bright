...
<html>
    ...
    <body>
        <div class="sticky-footer" style="min-height: 500px;">
            <%@include file="../../common/interface/fragments/header.jspf"%>
            <header class="container">
                <h2>
                    <% if(currentCategory != null) {%>
                        <%= currentCategory.getName()%>:
                    <% }%> <%= customerRequest.getTemplateName()%> - Submitted
                </h2>
                <hr class="soften">
            </header>
            <section class="container">
                <div id="pageQuestionsForm" class="border rounded">
                    <p style="margin-top: 20px;">
                        <b>Thank you for submitting your update.
                        </b>
                    </p>
                    <p>You will return to the <a href="<%=bundle.getProperty("submissionsUrl")%>">My History Page</a> in <span id="redirectCounter">3</span> second(s).</p>
                </div>
            </section>
    ...
    <script type="text/javascript">
        ...
        {
            var counter = parseInt($('#redirectCounter').text());
            setInterval(function()
            {
                counter--;
                $('#redirectCounter').text(counter);
                if (counter == 0)
                {
                    window.location.href = '<%=bundle.getProperty("submissionsUrl")%>';
                }
            }, 1000);
        });
    </script>
